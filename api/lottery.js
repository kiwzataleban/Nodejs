...
router.put("/prize", (req, res) => {
    let prize = req.body;
    const sqlUpdateLotto = "UPDATE lottory SET accepted = ? WHERE lottery_id = ? AND uid = ?";
    conn.query(sqlUpdateLotto, [1, prize.lid, prize.uid], (err, result) => {
        if (err) {
            console.error(err);
        ...
        conn.query(sqlCheckWallet, [prize.uid], (err, result) => {
            ...
            let currentWallet = result[0].balance;
            let newBalance = currentWallet + prize.money;
            const sqlUpdateWallet = "UPDATE users SET balance = ? WHERE uid = ?";
            conn.query(sqlUpdateWallet, [newBalance, prize.uid], (err) => {
                if (err) {
                    console.error(err);
                    return res.status(500).send("Internal Server Error");
                }
